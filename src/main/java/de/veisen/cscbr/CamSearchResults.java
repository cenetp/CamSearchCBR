...
public class CamSearchResults extends HttpServlet {
  ...
  private int casesCount = 5;

  public CamSearchResults() {
   ...
   */
  private ArrayList<Hashtable<String, String>> parseCSQuery(String autoMode, String builtInFlash,
      String dustReduct,
      int effPix, int focusPoints, int ISOmax, String imageRatio, String imageStab, String liveView,
      String manufact, String multLang, String sensorType, String sensorClass, String storageType,
      String video,
      String viewfinder, int weight, int whiteBal, String wifi) {
    ...
    SymbolDesc sensorClassSym = (SymbolDesc) concept.getAllAttributeDescs()
        .get("Sensor class (size)");
    ...
    IntegerDesc effPixSym = (IntegerDesc) concept.getAllAttributeDescs()
        .get("Effective pixels (million)");
    ...
    IntegerDesc whiteBalSym = (IntegerDesc) concept.getAllAttributeDescs()
        .get("White balance levels");
    ...
    SymbolDesc imageStabSym = (SymbolDesc) concept.getAllAttributeDescs()
        .get("Image_stabilization_(body)");
    SymbolDesc liveViewSym = (SymbolDesc) concept.getAllAttributeDescs().get("Live_view");
    SymbolDesc multLangSym = (SymbolDesc) concept.getAllAttributeDescs()
        .get("Multiple_interface_languages");
    ...
    if (resultList.size() > 0) {
      ...
      for (int i = 0; i < casesCount; i++) {
        resultTable
            .add(getAttributes(resultList.get(i), prj.getConceptByID(CBRInit.getConceptName())));
   ...
   */
  private static Hashtable<String, String> getAttributes(Pair<Instance, Similarity> r,
      Concept concept) {
  ...
  private void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    ArrayList<Hashtable<String, String>> queryResult = parseCSQuery(
        request.getParameter("automodes"),
        request.getParameter("flash"), request.getParameter("dustreduct"), effPixInt,
        focusPointsInt, ISOmaxInt,
        request.getParameter("imageratio"), request.getParameter("imagestab"),
        request.getParameter("liveview"),
        request.getParameter("manufact"), request.getParameter("multlang"),
        request.getParameter("sensortype"),
        request.getParameter("sensorclass"), request.getParameter("storagetype"),
        request.getParameter("video"),
        request.getParameter("viewfinder"), weightInt, whiteBalInt, request.getParameter("wifi"));
    ...
    try (PrintWriter out = response.getWriter()) {
      ...
      out.println("<br> automodes: " + request.getParameter("automodes") + " " + "<br> flash: "
          + request.getParameter("flash") + " " + "<br> dust reduct: " + request
          .getParameter("dustreduct")
          + " " + "<br> eff pix: " + effPixInt + " " + "<br>focus points: " + focusPointsInt + " "
          + "<br> iso max: " + ISOmaxInt + " " + "<br> image ratio: " + request
          .getParameter("imageratio")
          + " " + "<br> stab: " + request.getParameter("imagestab") + " " + "<br> lv: "
          + request.getParameter("liveview") + " " + "<br> manufact: " + request
          .getParameter("manufact")
          + " " + "<br> multlang: " + request.getParameter("multlang") + " " + "<br> stype: "
          + request.getParameter("sensortype") + " " + "<br> sclass: " + request
          .getParameter("sensorclass")
          + " " + "<br> storage: " + request.getParameter("storagetype") + " " + "<br> video: "
          + request.getParameter("video") + " " + "<br> vf: " + request.getParameter("viewfinder")
          + " "
          + "<br> weight: " + weightInt + " " + "<br> white balance: " + whiteBalInt + " "
          + "<br> wifi: "
